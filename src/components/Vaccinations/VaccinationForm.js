...
class VaccinationForm extends React.Component {
  constructor(props) {
    ...
    if (vaccination) {
      const date = convertTimestampToISODate(vaccination.date);
      const expiration = convertTimestampToISODate(vaccination.expiration);
      this.state = { ...vaccination, date, expiration };
    } else {
      this.state = {
        name: "",
        date: "",
  ...
  handleSubmitClick(e) {
    ...
    const expiration = convertDateToTimestamp(this.state.expiration);
    formHandler({ ...this.state, date, expiration });
  }
...
export default withRouter(VaccinationForm);
